...
class MeetupViewTests(TestCase):
    def setUp(self):
        ...
        rsvp = models.RSVP(meetup=self.past_meetup,
                           remote_username='remote_user',
                           remote_uid='remote_uid', source='meetupcom',
                           status='coming')
        rsvp.save()
    ...
    def test_view_not_existing(self):
        ...
        """
        self.assertTrue(isinstance(self.client.get('/meetups/123/'),
                                   HttpResponseNotFound))
        self.assertTrue(isinstance(self.client.get('/meetups/2015-05-01/'),
                                   HttpResponseNotFound))
    ...
    def test_view_existing_via_date(self):
        ...
        self.assertEqual(200, self.client.get(
            '/meetups/{}'.format(date)).status_code)
