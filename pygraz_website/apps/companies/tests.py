...
class CompanyTestsMixin(object):
    def setUp(self):
        self.approved = models.Company(
            name='Approved', website='http://company.com',
            address_line='address', postal_code='8010', city='Graz',
            country='AT', approved=True
        )
        self.approved.save()
        ...
        self.not_approved = models.Company(
            name='Not-Approved', website='http://company.com',
            address_line='address', postal_code='8010', city='Graz',
            country='AT', approved=False
        )
...
class ListingTests(CompanyTestsMixin, TestCase):
    def test_listing_approved(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(list(response.context['company_list']),
                         [self.approved])
    ...
    def test_listing_editor_logged_in(self):
        ...
        response = self.client.get('/companies/')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(list(response.context['unapproved_companies']),
                         [self.not_approved])
...
class CreationTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='user', password='password', email='e@mail.com'
        )
    ...
    def test_create_logged_out(self):
        ...
        self.assertRedirects(response,
                             '/accounts/signin/?next=/companies/submit/')
    ...
    def test_create_logged_in(self):
        ...
        response = self.client.post('/companies/submit/', {
            'name': 'My Company',
...
class UpdateViewTests(CompanyTestsMixin, TestCase):
    def test_login_required(self):
        ...
        self.assertRedirects(response,
                             '/accounts/signin/?next=/companies/update/1/')
